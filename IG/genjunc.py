...
#If user didn't specify parameters "stop-codons" and "framework shifts", then generate only productive sequences.
try:
    fr = int(sys.argv[4])
except:
    fr = 1
try:
 ...
 #Import databases with germline sequences.   
handle = open('./V.fasta')
...
nucleotides = 'agtc'
#Functions for 3' and 5' palindroms. 
def pal_3 (sequence):
...
def pal_5 (sequence):
    ...
    my_seq = str(Seq(pal).reverse_complement())
    return my_seq
#Function for N-nucleotides
def n_region ():
    n_reg = ''
    for n in range(random.randint(5, 20)):
        n_reg += random.choice(nucleotides)
    return n_reg
for n in range (int(sys.argv[1])): #Generate n sequences
    ...
    len_n1 = len(n1)
    if fr == 1:
        if len(V_dom) % 3 == 1: #Avoid frameshifts
    ...
    df.to_csv("./Annotations/" + str(n) + "ann.csv", encoding='utf-8', index=True) #Generate .csv files with sequence annotation.
    if stop == 1:
        V_dom = V_dom.replace("tag", "tac") #Replace stop-codons
